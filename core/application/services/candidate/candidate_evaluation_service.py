...
class CandidateEvaluationService:
    ...
    async def approve_candidate(self, candidate_id: str) -> KeyframeCandidate:
        ...
        # Check if another candidate is already approved for this shot
        approved_candidate = await self.get_approved_candidate_for_shot(candidate.shot_contract_id)
        if approved_candidate and approved_candidate.id != candidate_id:
            raise ValueError(f"Shot {candidate.shot_contract_id} already has an approved candidate.")
