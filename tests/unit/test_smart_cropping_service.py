...
@pytest.fixture
def service():
    with patch("infrastructure.providers.render.smart_cropping_service.YOLO"):
...
def test_fallback_when_cv2_fails(service):
    with patch("infrastructure.providers.render.smart_cropping_service.cv2.VideoCapture") as mock_vc:
        mock_cap = MagicMock()
