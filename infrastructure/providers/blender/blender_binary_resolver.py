...
class BlenderBinaryResolver:
    ...
    def resolve(blender_bin_path: str | None = None) -> str:
        ...
        if blender_bin_path and blender_bin_path.strip():
            ...
            if sys.platform == "win32" and not blender_path.name.lower().endswith(
                ".exe"
            ):
        ...
        if sys.platform == "win32":
            ...
            if blender_foundation.is_dir():
                ...
                for blender_dir in sorted(
                    blender_foundation.glob("Blender*"), reverse=True
                ):
