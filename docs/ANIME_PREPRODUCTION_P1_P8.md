...
The set must be generated by SELMA Labs rather than populated by manually
placing finished images in the repository:

```text
python -m cli.main preproduction golden-set --model-id <model-name> --model-revision <immutable-revision> --output output/preproduction/akira-golden-set.json
```

The configured provider receives the locked Character Bible references, Visual
Style Bible, deterministic seeds and prohibited traits. Candidate images and
their manifest remain unlocked until review and explicit human approval pass.
