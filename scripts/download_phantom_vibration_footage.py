...
OUT = ROOT / "motion" / "public" / "phantom-vibration" / "footage"
QUERIES = {
    "pocket_check": "person checking smartphone pocket notification",
    "phone_closeup": "smartphone notification close up hand",
    "daily_scroll": "person using smartphone street portrait",
    "confused_check": "confused person checking phone notification",
}
...
def choose_video(videos: list[dict], used: set[int]) -> dict:
    ...
    return min(candidates, key=lambda video: (abs((video.get("duration") or 10) - 12), -((video.get("width") or 0) * (video.get("height") or 0))))
...
def choose_file(files: list[dict]) -> dict:
    ...
    return min(candidates, key=lambda item: (abs((item.get("width") or 0) - 1080), abs((item.get("height") or 0) - 1920), item.get("file_size") or 0))
